  ...
  useEffect(() => {
    const fetchNotices = async () => {
      try {
        ...
        // Filter for 'student' and 'all'
        const filtered = res.data.filter(n => n.target_audience === 'all' || n.target_audience === 'student');
        setNotices(filtered);
      } catch (err) {
          ...
          <div className="card-body">
            {loading ? (
              ...
              <div className="text-center py-5">
                ...
                <p className="text-muted">No notices for students yet.</p>
              </div>
            )}
          </div>
        </div>
